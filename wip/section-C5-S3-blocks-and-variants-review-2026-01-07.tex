...
% Este arquivo: section-C5-S3-blocks-and-variants-review-2026-01-07.tex

% Status: dev | review | final | approved | deprecated
% Locação: WIP/ (ativo) | ARCHIVE/ (aprovado/descartado)

% Instruções:
% 1. Para compilar isoladamente: descomente as linhas 33-34 abaixo
% 2. Para integração em guide.tex: remova TODAS as linhas do preamble até \begin{document}
% 3. Compile: pdflatex section-C5-S3-blocks-and-variants-review-2026-01-07.tex
...
Section 5.3: CMS Block and Variant Notes\\\vspace{0.2cm}
{\small \bmsver{} --- WIP File section-C5-S3-blocks-and-variants}}
\author{Carlos ``Metal'' Nader}
\date{Version \fulldocversion{} | January 7, 2026}
...
% File Name: section-C5-S3-blocks-and-variants-review-2026-01-07.tex
...
% Narrative Completion: 80%
% Table Fill Status: 0%
% Integration Status: READY FOR INTEGRATION TARGET v0.2.3.0

% Notes:
% - Section focuses on ECM pod variants (ALQ-131/ALQ-184) vs IDIAS differences
% - Covers F-16 Block 40/42/50/52 + NATO variants + international operators
% - Critical safety differences between external pod and internal ECM systems
% - This section bridges CMS actuation (S2) with system-specific behavior (S4+)
...
\subsubsection{Frequency Band Selection}
\label{sec:C5-S3-S3-S3-band-selection}

\begin{itemize}

\item \textbf{External ECM Pod}:
\begin{itemize}
\item Frequency bands are automatically cycled during transmission (pod cycles through all five bands continuously)
\item Pilot selects transmission mode via XMIT knob but cannot manually select specific bands
\end{itemize}

\item \textbf{IDIAS}:
\begin{itemize}
\item Frequency bands are automatically selected based on RWR threat priority
\item RWR provides threat bearing and frequency to IDIAS
\item IDIAS jams the highest-priority threat band, adapting in real-time to threat changes
\item Pilot has \textit{zero manual band selection} (this is intentional: automatic selection is superior in dynamic threat environment)
\end{itemize}

\end{itemize}

\subsubsection{Avionics Protection Differences}
\label{sec:C5-S3-S3-S4-avionics-protection}

\begin{itemize}

\item \textbf{External ECM Pod}:
\begin{itemize}
\item Modes 1 and 2 via XMIT knob = Avionics Priority (FCR/TFR/HARM protected from jamming)
\item Mode 3 = Continuous Jam (all antennas transmit; may degrade own avionics)
\item Pilot must select mode based on mission (if relying on radar for fire control, use Mode 1 or 2; if radar not needed, use Mode 3 for maximum coverage)
\end{itemize}

\item \textbf{IDIAS}:
\begin{itemize}
\item AVNC mode = Avionics Priority (FCR/TFR/HARM protected; only AFT antenna transmits)
\item ECM mode = ECM Priority (both FWD and AFT antennas transmit; onboard avionics may be degraded)
\item Pilot cycles modes via CMS Left based on mission phase (use AVNC if conducting precision strike or radar SAM defense; use ECM if in sustained air combat)
\end{itemize}

\end{itemize}

% ============================================================================
% SUBSECTION 5.3.4: SUMMARY CROSS-REFERENCE TABLE
% ============================================================================
...
% ============================================================================
% SUBSECTION 5.3.5: OPERATIONAL NOTES AND SAFETY REMINDERS
% ============================================================================
